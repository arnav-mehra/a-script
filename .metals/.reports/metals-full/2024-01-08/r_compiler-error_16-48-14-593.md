...
object ControlFlowParser extends JavaTokenParsers {
    ...
    def function_block:Parser[(String, List[String])] = "fn " ~ statement ~ block ^^ {
        case "while "~s~b => (s, b)
        case default => ("", List())
    }

    def while_block:Parser[(String, List[String])] = "while " ~ statement ~ block ^^ {
        case "while "~s~b => (s, b)
        case default => ("", List())
    }

    def if_block:Parser[(String, List[String])] = "if " ~ statement ~ block ^^ {
        case "if "~s~b => (s, b)
        case default => ("", List())
    }

    def block:Parser[List[String]] = "{" ~ rep(line) ~ "}" ^^ {
        case "{"~lst~"}" => lst
        case default => List()
    }
