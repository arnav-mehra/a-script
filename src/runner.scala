...
object Functions {
    class Pair(var p: Program, var c: Context)
    val data: HashMap[String, Pair] = HashMap()

    def get(n: String): Pair = {
        if (!data.contains(n)) {
            data(n) = Pair(a => Data.Number(0), Context())
        }
        data(n)
    }

    def add(n: String, p: Program, c: Context) = {
        if (!data.contains(n)) {
            data(n) = Pair(p, c)
        }
        data(n).p = p
...
object Runner {
    def genProgram(bt: Tree, ctx: Context): Program = {
        val ast_ls: AstList = genAstList(bt, ctx)
    ...
    def genAstList(bt: Tree, ctx: Context): AstList = {
        bt.map(a => genAst(a, ctx)).collect { case Some(v) => v }
    }

    def genAst(bn: Node, ctx: Context): Option[Ast] = {
        bn match {
            case Node.Fn(v, ps, bt) => {
                val ctx = Context()
                ps.foreach(p => ctx.get_var(p))
                val p: Program = genProgram(bt, ctx)
                Functions.add(v, p, ctx)
                None
            }
            case Node.Call(n, bt) => {
                val args = genAstList(bt, ctx)
            ...
            case Node.Var(v) => {
                val vi = ctx.get_var(v)
                Some(() => Env.vars(vi))
            }
            case Node.Print(v_bn) => {
                val Some(v) = genAst(v_bn, ctx): @unchecked
                Some(() => {
                    println(v())
                    Data.Number(0)
                })
            ...
            case Node.Set(v, fs_bt, op, e_bn) => {
                val vi = ctx.get_var(v)
                val Some(e) = genAst(e_bn, ctx): @unchecked
                val fs = genAstList(fs_bt, ctx)
            ...
            case Node.BinOp(e1_bn, op, e2_bn) => {
                val Some(e1) = genAst(e1_bn, ctx): @unchecked
                val Some(e2) = genAst(e2_bn, ctx): @unchecked
                op match {
                    case "+"  => Some(() => e1() +  e2())
                    case "-"  => Some(() => e1() -  e2())
                    case "*"  => Some(() => e1() *  e2())
            ...
            case Node.If(c_bn, bt) => {
                val Some(c) = genAst(c_bn, ctx): @unchecked
                val b = genAstList(bt, ctx)
                Some(() => {
    ...
    def run(code: String): Unit = {
        ...
        val res: Tree = ProgramParser.parse(mcode)
        println(res)

        val ctx = Context()
        val p: Program = genProgram(res, ctx)
